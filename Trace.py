...
class Trace:
  ...
  def __init__(self, trace_path=None):
    ...
    if self.trace_path == None:
      raise Exception("A trace file is needed to initialize a POT_Trace object")
    ...
    if self.tracing_utility == "strace":
      self.parser = POT_Strace_Parser.StraceParser(trace_path, syscall_definitions)
    elif self.tracing_utility == "truss":
      self.parser = POT_Truss_Parser(trace_path, syscall_definitions)
    else:
    ...
    #- in bundle can store metadata what command / date / OS / etc the trace was gathered from.
